...
class TextureVK : ITexture
{
	...
	public void Create(VkImage handle, VkImageAspectFlags aspectFlags, VkImageType imageType, VkExtent3D extent, Format format)
	{
		...
		m_TextureType = Sphenery.RHI.Vulkan.GetTextureType(imageType);
		m_Handles[0] = handle;
	...
	public Result Create(TextureDesc textureDesc)
	{
		...
		m_SampleCount = Sphenery.RHI.Vulkan.GetSampleCount(textureDesc.sampleNum);

		readonly VkImageType imageType = Sphenery.RHI.Vulkan.GetImageType(textureDesc.type);
		...
		info.format = Sphenery.RHI.Vulkan.GetVkFormat(m_Format);
		...
		m_ImageAspectFlags = Sphenery.RHI.Vulkan.GetImageAspectFlags(textureDesc.format);
	...
	public Result Create(TextureVulkanDesc textureDesc)
	{
		...
		m_MipNum = textureDesc.mipNum;
		m_ArraySize = textureDesc.arraySize;
		m_Format = Sphenery.RHI.VKFormatToNRIFormat((.)(VkFormat)textureDesc.vkFormat);
		m_ImageAspectFlags = (VkImageAspectFlags)textureDesc.vkImageAspectFlags;
		m_TextureType = Sphenery.RHI.Vulkan.GetTextureType((VkImageType)textureDesc.vkImageType);
